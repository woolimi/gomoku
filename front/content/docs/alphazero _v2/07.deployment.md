...
- **`start_iteration` caveat.** Configs include `priority_replay.start_iteration` (typically `15`), but in the current implementation PER activation is controlled by `priority_replay.enabled`; `start_iteration` is present in schema/config but not wired as an automatic runtime gate.
