...
The null window $[\alpha, \alpha+1]$ is a width-1 window — the standard formulation. In negamax engines this is written as `(-alpha-1, -alpha)`; this engine uses non-negamax minimax with explicit `isMaximizing`, so the equivalent is passing `(alpha, alpha+1)` directly. The probe can only produce a fail-low ($\leq \alpha$, move is not better) or a fail-high ($\geq \alpha+1$, move might be better).
...
int pvs(Board *board, int depth, int alpha, int beta, int currentPlayer,
  ...
  for (size_t i = 0; i < scored.size(); ++i) {
    ...
    if (firstChild) {
      ...
      score = pvs(board, depth - 1, alpha, alpha + 1, next,
                  mv.first, mv.second, !isMaximizing, evalFn);
...
PVS was implemented with help from the [chessprogramming.org PVS article](https://www.chessprogramming.org/Principal_Variation_Search) for the null-window re-search logic. The reference pseudocode uses negamax with `(-alpha-1, -alpha)`; this engine's non-negamax minimax form passes `(alpha, alpha+1)` directly — the same width-1 null window, just without the negation convention.
