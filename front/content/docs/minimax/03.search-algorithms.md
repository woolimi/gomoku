...
Benchmarks were measured on an École 42 lab Dell workstation (Intel ..., RAM ..., Ubuntu ...), using release build and single-thread execution.

On an opening position (~200 legal moves), plain alpha-beta at depth 5 takes ~4s. PVS with transposition table and killer moves reaches depth 10 in ~2.3s — the optimizations don't just enable deeper search, they make it faster than shallow search without them. As the board fills and the branching factor drops, response times fall well under a second. Medium difficulty guarantees the ~500ms response target by its 0.4s time limit; Hard exceeds it on early moves but converges quickly as the game progresses.
