...
Double-Three Prohibition and the capture rule can be configured in the **Settings** when using `minimax`. In `alphazero` mode, those rule options are fixed to the model defaults, and only the first player setting can be changed.
