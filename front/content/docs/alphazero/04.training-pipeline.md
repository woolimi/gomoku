...
Known deviation: config files include `priority_replay.start_iteration`, but the current trainer path activates PER directly from `priority_replay.enabled` (the `start_iteration` field is currently not wired as an automatic runtime switch).
