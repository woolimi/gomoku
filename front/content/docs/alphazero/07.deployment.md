...
**Day 6 (iter 138–150): Rule bug and recovery.** A double-three rule asymmetry was discovered: the C++ extension was only enforcing the forbidden move rule for Black, not White. This meant all training data had been generated under incorrect rules. The fix was applied, the entire replay buffer was purged, and training resumed from the current checkpoint with fresh self-play data under the corrected rules.
...
- **200 MCTS simulations per move** — strong CPU play with typical latency around ~2.5s per move on the current VM
...
Source: [`alphazero/server/engine.py:230-255`](https://github.com/sungyongcho/gomoku/blob/main/alphazero/server/engine.py#L230-L255)
...
ONNX inference is already supported and configurable for deployment (for example via `ALPHAZERO_INFER_BACKEND`), while the default path remains local PyTorch inference. Potential technical improvements include deeper ONNX benchmarking/tuning and model distillation to a smaller network for lower latency.
