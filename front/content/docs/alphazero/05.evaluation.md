...
In the current V4 evaluation configs (day7/day8), arena games use 1000 MCTS simulations per move with 6 stochastic opening turns (`eval_opening_turns = 6`, `eval_temperature = 1.0`, `eval_dirichlet_epsilon = 0.2`), then continue with deterministic move selection. The 600-search setting applies to baseline matches (`baseline_num_searches = 600`), not the main challenger-vs-champion arena.
...
where $p_0$ is the null hypothesis win rate (challenger is not better) and $p_1$ is the alternative hypothesis (challenger is better). In implementation, $n$ and $w$ are computed in two modes: ignore draws (`n = wins + losses, \, w = wins / n`) or include draws (`n = wins + losses + draws, \, w = (wins + 0.5 \cdot draws) / n`).
