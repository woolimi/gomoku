...
`alphazero/gomoku/scripts/train.py`
...
# Sequential
python gomoku/scripts/train.py --config configs/config_alphazero_test.yaml --mode sequential --device cpu

# Vectorized (single process, multiple game slots)
python gomoku/scripts/train.py --config configs/config_alphazero_vectorize_test.yaml --mode vectorize --device cpu

# Multiprocessing
python gomoku/scripts/train.py --config configs/config_alphazero_mp_test.yaml --mode mp --device cpu

# Ray
pip install -e ".[ray,torch-cpu]" --extra-index-url https://download.pytorch.org/whl/cpu
python gomoku/scripts/train.py --config configs/5x5_local_test.yaml --mode ray --device cpu
```
