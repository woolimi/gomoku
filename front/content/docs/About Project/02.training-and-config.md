...
- `alphazero/infra/cluster/restart_cluster.sh`
  ...
  - For the full list, set all `GCP_*` entries in `.env.example` (including `GCP_PROJECT`, `GCP_REGION`, `GCP_ZONE`, `GCP_REPO`, `GCP_CLUSTER_NAME`, `GCP_SSH_USER`, `GCP_CONTAINER_NAME`, `GCP_GPU_TAG`, `GCP_CPU_TAG`, `GCP_SA_NAME`, `GCP_HEAD_RESERVATION`, `GCP_SSH_PRIVATE_KEY`).
