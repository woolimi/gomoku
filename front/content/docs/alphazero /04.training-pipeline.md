...
<!-- DIAGRAM_05: D4 symmetry augmentation. Show a 5×5 board with an asymmetric stone pattern (e.g., three stones forming an L-shape at fixed positions, with a distinct marker stone to make orientation obvious). Arrange 8 panels in a 2×4 grid: top row = 4 rotations of the original (0°, 90° CCW, 180°, 270° CCW); bottom row = horizontal flip of each rotation (flip+0°, flip+90°, flip+180°, flip+270°). Label each panel with its transform. Show how stone positions change under each operation so the viewer can trace a specific stone moving through all 8 positions. Annotate: "one of these 8 is sampled randomly at dataset load time — ×8 free training samples from each self-play position, no extra compute." -->
<a href="/images/diagrams/DIAGRAM_05.png" target="_blank" rel="noopener noreferrer">
  <img src="/images/diagrams/DIAGRAM_05.png" alt="D4 symmetry augmentation" class="cursor-zoom-in" />
</a>
