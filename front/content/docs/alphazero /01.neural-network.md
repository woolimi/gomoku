...
<!-- DIAGRAM_02: Neural network architecture flowchart, top-to-bottom. Input (13×19×19 tensor — 13 planes covering stone occupancy×2, empty, last move, capture ratios×2, color indicator, forbidden mask, 5 move-history planes) → Start Block (Conv2d 3×3: 13→128ch, BatchNorm2d, ReLU; output: 128×19×19) → Residual Tower ×12 (each block: Conv 3×3 128→128 + BN + ReLU + Conv 3×3 128→128 + BN + skip-add + ReLU; output stays 128×19×19). Then split into two heads. Policy Head (green): Conv 1×1: 128→2ch, BN, ReLU → Flatten (2×19×19=722) → Linear 722→361 → 361 policy logits (masked for illegal moves + softmax during search). Value Head (purple): Conv 1×1: 128→1ch, BN, ReLU → Flatten (1×19×19=361) → Linear 361→256 + ReLU → Linear 256→1 → Tanh → scalar in [-1, +1] (+1 = current player wins). Color the shared backbone in blue, policy path green, value path purple, output nodes in distinct shades. -->
