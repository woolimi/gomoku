...
2. **Expand**: When a leaf node is reached (a node with no children), expand it by creating child nodes for all legal moves. Child priors come from policy logits after legal-move masking and softmax normalization.

3. **Evaluate**: Run neural network inference on the leaf state to get policy logits and a value estimate. The logits are converted to a legal-only prior distribution with softmax (and root Dirichlet noise during self-play).
