...
<!-- DIAGRAM_01: AlphaZero training loop cycle. Show five nodes forming a directed cycle: Self-Play (champion generates games with MCTS + Dirichlet noise; first 20 moves temperature=1.0; opponent mix: 65% champion, 30% previous champion, 5% random) → Replay Buffer (stores (state, MCTS policy, outcome) tuples; max 500K with FIFO eviction; D4 symmetry applied at load time giving 8× data views) → Training (2 epochs/iteration; batch 512; Adam + AMP; combined policy cross-entropy + value MSE loss; scheduled LR) → Evaluation Arena (40 head-to-head games, challenger vs champion, alternating Black/White; deterministic: temperature=0, no Dirichlet) → Promotion Gate (55% win-rate threshold: Yes → update champion and loop back to Self-Play; No → discard challenger). Show Serving as a dashed branch off the Champion node (FastAPI + WebSocket; 20 MCTS simulations; native C++ engine; stateless per request). -->
...
- 215 training iterations across 4 training runs on GCP Ray cluster
